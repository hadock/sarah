...
class mongodb_lib
{
	...
	* result_array.
	*
	* Return the documents found in an associative array
	*
	* <code>
	* $this->mongo_db->get('foo')->result_array();
	* </code>
	*
	*
	* @access public
	...
	*/
        public function result(){
            return json_decode(json_encode($this->documents));
        }
	...
	*/
        public function row(){
            if(count($this->documents)){
                return json_decode(json_encode($this->documents[0]));
            }else{
	...
	*/
	...
	private function _connect()
	{
		...
		catch (MongoConnectionException $exception)
		{
			...
			{
				$this->_show_error('Unable to connect to MongoDB: ' . $exception->getMessage()." <br>Connection string: ".$this->_connection_string."<br>Options: ".json_encode($options), 500);
			}
		}
	}
